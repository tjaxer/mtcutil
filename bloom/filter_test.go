...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := mtcutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(mtcutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := mtcutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := mtcutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
