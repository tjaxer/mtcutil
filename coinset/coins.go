...
func (cs *CoinSet) TotalValue() (value mtcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue mtcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue mtcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount mtcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue mtcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue mtcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue mtcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue mtcutil.Amount, coins []Coin) (Coins, error) {
...
// mtcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() mtcutil.Amount {
	return mtcutil.Amount(c.txOut().Value)
}
